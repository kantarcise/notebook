...
class Solution:
    def missingNumber_first_try(self, nums: list[int]) -> int:
    ...
    def missing_numbers(self, nums: list[int]) -> int:
        full_range = list(range(len(nums)+1))
        big_list = nums + full_range
        result = None
        for elem in big_list:
            result ^= elem
        return result
    
    def missingNumber_fellow(self, nums: list[int]) -> int:
