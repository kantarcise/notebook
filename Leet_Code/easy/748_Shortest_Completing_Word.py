"""
Given a string licensePlate and an array of strings 
words, find the shortest completing word in words.

A completing word is a word that contains all the letters in licensePlate. 
Ignore numbers and spaces in licensePlate, and treat letters as case 
insensitive. If a letter appears more than once in licensePlate, then 
it must appear in the word the same number of times or more.

For example, if licensePlate = "aBc 12c", then it contains 
letters 'a', 'b' (ignoring case), and 'c' twice. Possible completing words 
...
or do some str.count("c") shenenigans
"""

from  collections import Counter

class Solution:
    def shortestCompletingWord_(self, licensePlate: str, words: List[str]) -> str:
        # this works, but it is pretty slow
        words_plate = []
        for i in licensePlate.lower():
            if i.isalpha():
                words_plate.append(i)
        
        plate = Counter(words_plate)
        res = []
        for word in words:
        ...
        res.sort(key = len) # this returns None
        
        # smallest one to be returned
        return res[0]
        
    def shortestCompletingWord(self, licensePlate: str, words: List[str]) -> str:
        
        # double operations in single line
        plate = ''.join([i.lower() for i in licensePlate if i.isalpha()])

        # sort the words list based on length
        words = sorted(words, key=len)
        # for every word
        # check every index on the plate, and if 
        # counts does not match,
        # abandon that word
        for word in words:
            for i in range(len(plate)):
                if word.count(plate[i]) < plate.count(plate[i]):
                    break
                # if they match characteristicly
                # and length wise
                if i == len(plate) - 1:
                    return word
