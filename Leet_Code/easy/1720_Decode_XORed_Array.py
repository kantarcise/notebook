...
Takeaway:
  ...
  XOR is the Apple operator.
  From left or from right, you can do an operation in a equation.

"""

class Solution:
    def decode(self, encoded: List[int], first: int) -> List[int]:
        # we know the first elem,
        # we know encoded
        # what is the hidden int ?
        
        # first ^ dec = enc
        # first ^ dec ^ dec = enc ^ dec
        # first ^ 0 = enc ^ dec
        # first = enc ^ dec
        
        # enc[i] = arr[i] ^ arr[i+1]
        # arr[i] ^ enc[i] = arr[i] ^ arr[i] ^ arr[i+1]
        
        # arr[i] ^ enc[i] = 0 ^ arr[i+1]
