...
to val be k, to get accepted, you need to do the following things:
  ...
  Change the array nums such that the first k elements of nums 
    contain the elements which are not equal to val. The 
    remaining elements of nums are not important as well 
    as the size of nums.
  Return k.
...
Takeaway:
  ...
  You can use Counter, which will absolutely work.

  Just simple indexing will work too.
...
class Solution:
    def removeElement_(self, nums: List[int], val: int) -> int:
        # this works, but uses a lot different methods
        c = Counter(nums)
        number_of_val = c[val]
        for i in range(number_of_val):
            nums.remove(val)
            
        return sum(c.values()) - c[val]
    
    def removeElement(self, nums: List[int], val: int) -> int:
        # we can move elements ourselves
        index = 0
